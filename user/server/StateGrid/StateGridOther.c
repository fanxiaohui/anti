...
static int jinlangProcess(DeviceCmd_t *cmd){
    ...
    u32 activePower3 = 0;
    u16 totalPower = 0;
    u16 currentVoltage = 0;
    u16 currentCurrent = 0;
...
static float time_character_analysis(u16_t* year,u8_t* month ,u8_t* day,u8_t* value)
{
    ...
    Ql_memset(year_buf,  0, ConnectionPlace1+1);
    Ql_memset(month_buf, 0, ConnectionPlace2-ConnectionPlace1);
    Ql_memset(day_buf,   0, cnt-ConnectionPlace2);

    Ql_memcpy(year_buf,    value,                      ConnectionPlace1);      
    Ql_memcpy(month_buf,   value+ConnectionPlace1+1,   ConnectionPlace2-ConnectionPlace1-1); 
    Ql_memcpy(day_buf,     value+ConnectionPlace2+1,   cnt-ConnectionPlace2-1); 
    ...
    *day   = Ql_atoi(day_buf);
    ...
    Ql_memset(value_buf,  0, end-space);
    Ql_memcpy(value_buf,value+space+1,end-space-1);
...
static float hongguang_time_comparison(float total_power)
{
    ...
    float hongguang_value = time_character_analysis(&hongguang_year,&hongguang_month,&hongguang_day,hg_time_hg_value.payload);
...
static int SpecialProcess(DeviceCmd_t * cmd)
{
    ...
    //艾伏1BE7 E8 E9数据处理
    //艾伏02E4
    if(!memcmp("02FF", Protocol.payload, strlen("02FF")) && !memcmp("89", CompanyID.payload, 2)){
        ret = aifuProcess(cmd);
    }
    else if(!memcmp("02FF", Protocol.payload, strlen("02FF")) && !memcmp("55", CompanyID.payload, 2)){
        ret = jinlangProcess(cmd);
    }
    else if(!memcmp("02FF", Protocol.payload, strlen("02FF")) && !memcmp("13", CompanyID.payload, 2)){
        ret = hongguangProcess(cmd);//竑光
    }    
    else if(!memcmp("02FF", Protocol.payload, strlen("02FF")) && !memcmp("77", CompanyID.payload, 2)){
...
static void pointDataSys(u16_t number, DeviceCmd_t *cmd)
{
    ...
    {
        lenght = StateGrid_pointValueLenght(otherCollecTab->tab[i].src->type, otherCollecTab->tab[i].src->lenght,  otherCollecTab->tab[i].src->rsv);
        ...
        if(otherCollecTab->tab[i].src->id == 288549063)
            APP_DEBUG("type = %d, id = %d, lenght=%d, size = %d, valueLength = %d\r\n",
...
//                        APP_DEBUG("var = %02X, offset = %d\r\n", val, offset);
                ...
                else
                {
                    if ((otherCollecTab->tab[i].src->lenght&0x40) == 0x40) //有符号
                    {
                        ...
                        {
                            val = (s32_t)((cmd->ack.payload[offset+3]<<24)|(cmd->ack.payload[offset+2]<<16)|(cmd->ack.payload[offset+1]<<8)|cmd->ack.payload[offset]);
                        }
                        ...
                        {
                            val = (s32_t)((cmd->ack.payload[offset+1]<<24)|(cmd->ack.payload[offset]<<16)|(cmd->ack.payload[offset+3]<<8)|cmd->ack.payload[offset + 2]); 
                        }
                        else //高位寄存器在前-寄存器大端
                        {
                    ...
                    else
                    {
                        ...
                        {
                            val = (u32_t)((cmd->ack.payload[offset+3]<<24)|(cmd->ack.payload[offset+2]<<16)|(cmd->ack.payload[offset+1]<<8)|cmd->ack.payload[offset]);
                        }
                        else if ((otherCollecTab->tab[i].src->lenght&0x30) == 0x20)//高位寄存器在前-寄存器小端
                        {
                            val = (u32_t)((cmd->ack.payload[offset+1]<<24)|(cmd->ack.payload[offset]<<16)|(cmd->ack.payload[offset+3]<<8)|cmd->ack.payload[offset + 2]); 
                        }
                        else//高位寄存器在前-寄存器大端
                        {
            ...
            {
                r_memcpy(otherCollecTab->tab[i].src->value.payload, &cmd->ack.payload[offset], otherCollecTab->tab[i].src->value.lenght);
