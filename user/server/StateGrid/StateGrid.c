...
{
    {0x00, stateGrid_loginAck},             //閻ц缍�
    {0x01, stateGrid_registerAck},          //濞夈劌鍞介崢鍌滅彲
    {0x04, stateGrid_uploadAck},            //娑撳﹥濮ょ€圭偞妞傞弫鐗堝祦
    {0x05, stateGrid_historyUploadAck},     //娑撳﹥濮ら弬顓犲仯缂侇厺绱堕弫鐗堝祦
    {0x10, stateGrid_getData},              //閸欘剚绁寸€圭偞妞傞弫鐗堝祦
    {0x21, stateGrid_prooftime},            //鐎佃妞�
    {0x99, stateGrid_heartbeatAck}          //闁炬崘鐭惧ù瀣槸閿涘牆绺剧捄绛圭礆
};
/*******************************************************************************
  * @note   None
  * @param  None
...
{
    step = 0;
    uploadDataSpace = (3*60/SEND_SPACE); //閸ョ晫缍夋稉濠佺炊鐎圭偞妞傞弫鐗堝祦闂傛挳娈ч弮鍫曟？
    heartbeatSpace = 0;
    historySapce = 0;
    list_init(&rcveList);
    StateGrid_dataCollect();
...
static void StateGrid_process(void)
{
    ...
    if (sp != null)
    {		
        StateGridTab_t *exe = (StateGridTab_t*)ALG_binaryFind(sp->cmd->func, (void*)stateGridCmdTab, (void*)(&stateGridCmdTab[sizeof(stateGridCmdTab)/sizeof(stateGridCmdTab[0])]), sizeof(stateGridCmdTab[0]));
 ...
 * @return
 */
static int getStationTab(Number_t *ST, char *para)
{
    ListHandler_t NumberStr;

    APP_DEBUG("State Grid %s\r\n", para);
    r_strsplit(&NumberStr, para, ';');
    ST->count = NumberStr.count;    //閼惧嘲褰囬幀璇插彙閻ㄥ嫮鏁哥粩娆愭殶闁诧拷, 閹粯鏆熸稉宥堝厴鐡掑懓绻僑TATION_COUNT=4
    ...
    else
    {
        ...
        while (station != null && i < STATION_COUNT) 
        {
            ...
            ST->No[i].number = Swap_charNum((char*)*value);  //閹绘劒姘﹂悽鐢电彲ID
            addr = ST->No[i].addr;
...
u16_t StateGrid_station(u8_t addr, u32_t *No)
{
    ...
    for (i = 0; i < Num.count; i++)
    {
        ...
        do 
        {
            ...
            APP_DEBUG("State Grid number addr get %d, %d\r\n",Num.No[i].number, Num.No[i].addr[n]);
...
#pragma pack() 
    ...
    if (sg->cmd->pdu.lenght == sizeof(loginAck_t))
    {
        ...
        {
            log_saveAbnormal("State Grid login ERR:", ack->result);
        }
    }
    else
    {
        log_saveAbnormal("State Grid login ack lenght ERR ", sg->cmd->pdu.lenght);
    }
...
#pragma pack() 
    ...
    if (sg->cmd->pdu.lenght == sizeof(registerAck_t))
    {
        ...
        {
            log_saveAbnormal("State Grid register ERR:", ack->result);
        }
    }
    else
    {
        log_saveAbnormal("State Grid login ack register ERR ", sg->cmd->pdu.lenght);
    }
...
#pragma pack() 
    ...
    if (sg->cmd->pdu.lenght == sizeof(uploadAck_t))
    {
        ...
        if (ack->result == 0x01)
        {
            ...
            // log_saveAbnormal("State Grid upload data OK :", ack->result);
        }
        else
        {
            log_saveAbnormal("State Grid upload data ERR:", ack->result);
        }
    }
    else
    {
        log_saveAbnormal("State Grid upload data ERR ", sg->cmd->pdu.lenght);
    }
...
#pragma pack() 
    ...
    if (sg->cmd->pdu.lenght == sizeof(uploadAck_t))
    {
        ...
        {
            log_saveAbnormal("State Grid history upload data ERR:", ack->result);
        }
    }
    else
    {
        log_saveAbnormal("State Grid history upload data ERR ", sg->cmd->pdu.lenght);
    }
...
ERR1:
    ...
    return 0;
ERR:
    log_saveAbnormal("State get rcveice lenght ERR ", sg->cmd->pdu.lenght);
    return 0;
}
/*******************************************************************************
...
#pragma pack() 
    ...
    {
        log_saveAbnormal("State proof time rcveice lenght ERR ", sg->cmd->pdu.lenght);
        ack.result = 0x10;
    }
...
#pragma pack() 
    ...
    if (sg != null && sg->cmd->pdu.lenght == sizeof(heartbeatAck_t))
    {
        ...
        {
            log_saveAbnormal("State Grid heartbeat ERR:", ack->code);
        }
    }
