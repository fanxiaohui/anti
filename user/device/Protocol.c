...
const ST_UARTDCB UART_2400_N1 	= {2400, 	DB_8BIT, SB_ONE, PB_NONE, FC_NONE};
const ST_UARTDCB UART_4800_N1 	= {4800, 	DB_8BIT, SB_ONE, PB_NONE, FC_NONE};
const ST_UARTDCB UART_9600_N1 	= {9600, 	DB_8BIT, SB_ONE, PB_NONE, FC_NONE};
const ST_UARTDCB UART_9600_E1 	= {9600, 	DB_8BIT, SB_ONE, PB_EVEN, FC_NONE};
const ST_UARTDCB UART_19200_N1 	= {19200, 	DB_8BIT, SB_ONE, PB_NONE, FC_NONE};
const ST_UARTDCB UART_38400_N1 	= {38400, 	DB_8BIT, SB_ONE, PB_NONE, FC_NONE};
const ST_UARTDCB UART_115200_N1 = {115200, 	DB_8BIT, SB_ONE, PB_NONE, FC_NONE};

const DeviceProtocol_t protocolTab[] = {
	{0x0200, DEVICE_MOBUS, &EybondDevice}, 
	...
	{0x0210, DEVICE_MOBUS, &MOSODevice},   
	{0x0211, DEVICE_MOBUS, &MOSODevice},   
	{0x0212, DEVICE_MOBUS, &MOSODevice},   
	...
	{0x0222, DEVICE_MOBUS, &ClouDevice},   
	...
	{0x0230, DEVICE_MOBUS, &InvtDevice},   
	...
	{0x0240, DEVICE_MOBUS, &SinengDevice}, 
	{0X0241, DEVICE_MOBUS, &SinengDevice}, 
    {0x0242, DEVICE_MOBUS, &_0242Device},
	{0x0243, DEVICE_MOBUS, &SinengDevice}, 
	{0x0244, DEVICE_MOBUS, &SinengDevice}, 
	{0x0245, DEVICE_MOBUS, &SinengDevice}, 
	...
	{0x0250, DEVICE_MOBUS, &EastDevice},  
	...
	{0x0260, DEVICE_MOBUS, &LittoDevice},	
	...
	{0x0270, DEVICE_MOBUS, &Kstar2Device},	
	{0x0272, DEVICE_MOBUS, &_0272Device},	
	{0x0273, DEVICE_MOBUS, &_0273Device},	
	...
	{0x0290, DEVICE_MOBUS, &GrowattDevice}, 
	...
	{0x029C, DEVICE_MOBUS, &CRRCDevice},
	{0x029D, DEVICE_MOBUS, &Innovpower9DDevice}, 
	{0x029E, DEVICE_MOBUS, &Sungrow029EDevice},   
	{0x029F, DEVICE_MOBUS, &SofarDevice},		
	...
	{0x02A5, DEVICE_MOBUS, &_02A5Device},
	{0x02A6, DEVICE_MOBUS, &_02A6Device},
	{0x02A7, DEVICE_MOBUS, &HuaweiDevice},		
	{0x02A8, DEVICE_MOBUS, &HuaweiDevice},			
	{0x02A9, DEVICE_MOBUS, &HuaweiDevice},	    
	...
	{0x02C6, DEVICE_MOBUS, &HuaweiDevice},		
	...
	{0x02D1, DEVICE_MOBUS, &_02D1Device}, 
	{0x02D2, DEVICE_MOBUS, &GreatWallEQMX3Device},
	{0x02D3, DEVICE_MOBUS, &HuaweiDevice},		
	{0x02D4, DEVICE_MOBUS, &_02D4Device},	
	{0x02D5, DEVICE_MOBUS, &Sungrow02D5Device},
	{0x02D7, DEVICE_MOBUS, &SenErgyDevice},
	{0x02D8, DEVICE_MOBUS, &ClenErgyDevice},    
	{0x02D9, DEVICE_MOBUS, &_02D9Device}, 
	{0x02DA, DEVICE_MOBUS, &NOGODevice}, 
	{0x02DB, DEVICE_MOBUS, &Tbea3Device}, 
	{0x02DC, DEVICE_MOBUS, &_02DCDevice},
	{0x02DD, DEVICE_MOBUS, &HuaweiDevice},		
	{0x02DE, DEVICE_MOBUS, &HuaweiDevice},		
	...
	{0x0721, DEVICE_MOBUS, &_0721Device},
	...
	{0x091B, DEVICE_MOBUS, &_091BDevice},	
	{0x091C, DEVICE_MOBUS, &_091CDevice},	
	...
	{0x0948, DEVICE_MOBUS, &_0948Device},
	...
	{0x0958, DEVICE_MOBUS, &_0958Device},
	{0x0959, DEVICE_MOBUS, &_0959Device},
	{0x0965, DEVICE_MOBUS, &_0965Device},
	{0x0971, DEVICE_MOBUS, &_0971Device},
	{0x0973, DEVICE_MOBUS, &_0973Device},
	{0x0975, DEVICE_MOBUS, &_0975Device},
	...
	{0x1201, DEVICE_MOBUS, &_1201Device},
	{0x1203, DEVICE_MOBUS, &_1203Device},
	{0x1205, DEVICE_MOBUS, &_1205Device},
	{0x1208, DEVICE_MOBUS, &_1208Device},
	{0x1209, DEVICE_MOBUS, &_1209Device},
};

static u8_t addrTab[68];
static const char trans[]  = "1";
static const char modbus[] = "0";
...
void Protocol_init(void)
{
	...
	if (pro != null)
	{
		i = MAX(attr.endAddr - attr.startAddr, ModbusDevice.monitorCount-1); //濡傛灉涓虹洃锟�?1鍙帮紝姝ゅ€间负0
		APP_DEBUG("Device protocol code %x, %d - %d. \r\n", attr.code, attr.startAddr, i + 1);

		/* 鍦板潃鍒楄〃涓啓鍏ュ疄闄呮墍闇€鐩戞帶鐨勮澶囧湴鍧€ */
		for (*p++ = attr.startAddr; i > 0 && p < (u8_t*)(&addrTab + 1); i--, p++)
		{
			*p = *(p-1) + 1;
		}
		...
		if (pro->type == DEVICE_MOBUS)
		{
			...
			ModbusDevice.cfg = attr.cfg;    //鎻愬彇涓插彛閰嶇疆鍙傛暟
			buf.payload = (u8_t*)modbus;
		}
		else if (pro->type == DEVICE_TRANS)
		{  
			SysPara_Get(DEVICE_VENDER, &buf);
			TransDevice_init((char *)buf.payload, attr.cfg);//*/
			memory_release(buf.payload);
			buf.payload = (u8_t*)trans;
		}
	}
...
int protocolAttrGet(u8_t num, ProtocolAttr_t *attr)
{
	...
	if (buf.payload != null && buf.lenght > 0)
	{
		...
		else if (attrStr.count > 3)
		{
			...
			attr->cfg = bandrateGet((char*)*(int*)attrStr.node->next->next->next->payload);
		}
		else
		{
			attr->code = Swap_HexCharNum((char*)*(int*)attrStr.node->payload);
...
/*******************************************************************************
  ...
  * @param	None
  * @retval None
  * @note   #9600-8-1-0#  #锟斤拷锟斤拷锟斤拷-锟斤拷锟斤拷位-停止位-校锟斤拷位#
*******************************************************************************/
static ST_UARTDCB * bandrateGet(char *str)
{
	...
	if (cmdStr.count == 3)
	{
		...
		{
			cfg = memory_apply(sizeof(ST_UARTDCB));
			cfg->baudrate = tab[0];
			cfg->dataBits = tab[1];
			cfg->stopBits = tab[2];
			cfg->parity = tab[3];
