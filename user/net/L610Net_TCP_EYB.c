  ...
  if (nIndex < sizeof(netManage) / sizeof(netManage[0])) {
    if (netManage[nIndex].flag == 1) {
      ...
      if (ret == L610_SUCCESS) {
        NetLED_On();
        GSMLED_On();      //硬件网络连接成功
        eybnet_para.inwork_status = 1; //全局变量，TCP连接成功
      } else {
        NetLED_Off();
        eybnet_para.inwork_status = 0; //全局变量，TCP连接失败
  ...
  switch(m_GprsActState) {
    case STATE_GSM_QUERY_STATE: {   // 查询SIM卡插入状态
      ...
      if ((simstatus == 1) && (simret == 0)) {
        ...
        Ql_OS_SendMessage(NET_TASK, NET_MSG_SIM_INSERTED, 0, 0,0);
...
//      APP_DEBUG("sim getRegInfo ret:%d, reg_state = %d, curr_rat=%d\r\n", ret, sim_reg_info.nStatus ,sim_reg_info.curr_rat);
    ...
    if(1 == sim_reg_info.nStatus) { // SIM卡已注册
      //注册成功
        ...
        if (ret == 0) {
          //激活PDP
          Ql_OS_SendMessage(NET_TASK, NET_MSG_SIM_READY, 0, 0,0);
    ...
    case STATE_SIM_NOT_INSERTED:
      Ql_OS_SendMessage(NET_TASK, NET_MSG_SIM_NOT_INSERTED, 0, 0,0);
      m_GprsActState = STATE_GSM_QUERY_STATE;
      break;
    case STATE_SIM_READY: { // 确认注网后获得IP
      ...
      if (0 == fibo_PDPStatus(1, ip, &cid_status, SINGLE_SIM)) {
         ...
         if (cid_status == 1 && r_strlen((char *)ip) != 0) {
           Ql_OS_SendMessage(NET_TASK, NET_MSG_GSM_READY, 0, 0,0);
    ...
    case STATE_SIM_NOT_READY:
      Ql_OS_SendMessage(NET_TASK, NET_MSG_SIM_FAIL, 0, 0,0);
      m_GprsActState = STATE_GSM_QUERY_STATE;
      break;
    case STATE_GSM_READY: {
      ...
      if (ret == 0) {
        ...
        if (ping_ret == 0) {
          ...
          Ql_OS_SendMessage(NET_TASK, NET_MSG_DNS_READY, 0, 0,0);
...
//        registe = 0;
    ...
    case STATE_GSM_NOT_READY:
      Ql_OS_SendMessage(NET_TASK, NET_MSG_GSM_FAIL, 0, 0,0);
      m_GprsActState = STATE_GSM_QUERY_STATE;
      break;
...
//              Ql_OS_SendMessage(EYBNET_TASK, NET_CMD_RESTART_ID, 0, 1);   // mike需要修改逻辑?
                //退出前，需g_netmutex = 0;
                g_netmutex = 0;
		        return;
              }
...
//          APP_DEBUG("nIndex %d socketID %ld is not ready\r\n", nIndex, netManage[nIndex].socketID);
    ...
    case STATE_DNS_NOT_READY: {
      ...
      m_GprsActState = STATE_GSM_QUERY_STATE;
      Ql_OS_SendMessage(NET_TASK, NET_MSG_DNS_FAIL, 0, 0,0);
...
static void realtime_chk_pdp(void)
{
  ...
  if (0 == fibo_PDPStatus(1, ip, &cid_status, SINGLE_SIM)) {
     ...
     APP_DEBUG("sim RAT mode after PDP active:%ld\r\n", ret);
     if (cid_status == 1 && r_strlen((char *)ip) != 0) {
       //Ql_OS_SendMessage(EYBNET_TASK, NET_MSG_GSM_READY, 0, 0,0);
       //m_GprsActState = STATE_GSM_READY;
       registe_times = 0;
     } else {           
       registe_times++;
